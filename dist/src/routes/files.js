...
exports.default = (io) => {
    ...
    router.post("/supporter-report-barangay", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            if (teamToInsert.length > 0) {
                ...
                teamToInsert.forEach((team) => {
                    var _a;
                    const teamLeader = ((_a = team.TeamLeader) === null || _a === void 0 ? void 0 : _a.voter)
                        ? `${(0, data_1.handleLevel)(team.TeamLeader.voter.level)} = ${team.TeamLeader.voter.idNumber} - ${team.TeamLeader.voter.lastname}, ${team.TeamLeader.voter.firstname} (${team.voters.length})`
                        : "No Leader";
                    const members = team.voters.map((voter) => `     ${voter.idNumber} - ${voter.lastname}, ${voter.firstname}`);
                    const membersTags = team.voters.map((voter) => `[${voter.status === 0 ? "D," : ""} ${voter.oor === "YES" ? "OR," : ""} ${voter.inc === "YES" ? "INC," : ""} ${(0, data_1.memberTags)(voter.VoterRecords.type)}]`);
        ...
        try {
            ...
            console.log({ data });
            barangayId ? doc.fontSize(16).text(`${barangay === null || barangay === void 0 ? void 0 : barangay.name}-(${(0, data_1.formatToLocalPHTime)(day)})`, {
                align: "left",
        ...
        try {
            ...
            doc.fontSize(10).text(`Percentage: ${(0, data_1.calculatePercentage)(survey._count.RespondentResponse, 1500)}%`, 20, 160);
            doc.moveDown(2);
            // Survey Queries
            queries
                ...
                if (query.Option) {
                    query.Option.forEach((option, j) => {
                        ...
                        doc.moveDown(0.5);
                        doc.fontSize(10).text(`Total: ${option.Response.length} (${(0, data_1.calculatePercentage)(option.Response.length, query.Response.length)}%)     Male: ${male} (${(0, data_1.calculatePercentage)(male, option.Response.length)}%) | Female: ${female} (${(0, data_1.calculatePercentage)(female, option.Response.length)}%)`, { indent: 60 });
                        doc.moveDown(0.5);
                    });
                }
                doc.moveDown(1);
